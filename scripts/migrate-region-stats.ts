...
async function verifyRegionStats() {
	...
	try {
		...
		const sortedStats = statsByLastPaintedAt.sort((a, b) => (b.lastPaintedAt as any) - (a.lastPaintedAt as any))
			.slice(0, 10);
		for (const stat of sortedStats) {
			console.log(`  ${new Date(stat.lastPaintedAt as any)
				.toISOString()}: ${stat._count.id.toLocaleString()}`);
		}
		...
		const withAlliance = statsByAlliance.filter(s => s.allianceId !== null)
...
try {
	...
	try {
		...
		throw new Error("UserRegionStats table not found. Please run 'npx prisma db push' first.");
	}
	...
	try {
		...
		throw new Error("UserRegionStatsDaily table not found. Please run 'npx prisma db push' first.");
	}
